"""
    MPNN code from , modified to work with 2D glasses data.  
...
# Import the GlassyData class

from graph_reader_class import GlassyData
...
# Add option for nearest neighbors
parser.add_argument('--num_neighbors', type=int, default=7, help='Number of nearest neighbors for graph construction')
...
#     args.cuda = not args.no_cuda and torch.cuda.is_available()
    ...
    print('Prepare files')
    metadata = [f for f in os.listdir(root) if os.path.isfile(os.path.join(root, f))]


    train_metadata, remaining_metadata = train_test_split(metadata, test_size=0.3, random_state=0)
    validation_metadata, test_metadata = train_test_split(metadata, test_size = 0.5, random_state = 0)

    
    data_train = GlassyData(root, train_metadata, args.num_neighbors)
    data_valid = GlassyData(root, validation_metadata, args.num_neighbors)
    data_test = GlassyData(root, test_metadata, args.num_neighbors)
